...
namespace MOCA.Services.Implementation.LocationManagment
{
    public class CityService : ICityService
    {
        ...
        public async Task<Response<long>> AddCity(CityModel request)
        {
            ...
            var entityCity = await _unitOfWork.CityRepoEF.IsUniqueNameAsync(request.CityName);
            if (entityCity == false)
            {
                return new Response<long>("City Name is not Unique.");
            }
        ...
        public async Task<Response<bool>> UpdateCity(CityModel request)
        {
            ...
            var entityCity = await _unitOfWork.CityRepoEF.IsUniqueNameAsync(request.CityName, request.Id);
            if (entityCity == false)
            {
                return new Response<bool>("City Name is not Unique.");
            }
        ...
        public async Task<Response<List<CityModel>>> GetAllCityWithoutPagination()
        {
            ...
            if (Res.Count == 0)
            {
                return new Response<List<CityModel>>(null, "Not Data Found");
            }
